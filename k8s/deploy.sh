...
# Skip cluster creation - assume it's handled by pipeline
echo "� Using existing Kind cluster..."

# Set the kubectl context
kubectl config use-context kind-ecommerce || echo "⚠️ Context not set, continuing anyway..."

# Simple readiness check with fallback
echo "🔌 Checking cluster connectivity..."
if ! kubectl get nodes &>/dev/null; then
    echo "⚠️ Cluster not fully ready, waiting 30 seconds..."
    sleep 30
fi
